// Created by Fame (fa-e) and released to the public domain.
...
static inline void load_leds_mode()
{
    ...
    if (!is_in_normal_mode() && !is_in_special_mode())
        leds_mode = LEDS_MODE_NORMAL_MIN;
}
...
static void load_bat_levels()
{
    ...
    if (bat_empty_level == 0xFF)
        bat_empty_level = DEFAULT_BAT_EMPTY;

    bat_min_level = eeprom_read_byte(EEPROM_BAT_MIN_ADDR);
    if (bat_min_level == 0xFF)
        bat_min_level = DEFAULT_BAT_MIN;

    bat_low_level = eeprom_read_byte(EEPROM_BAT_LOW_ADDR);
    if (bat_low_level == 0xFF)
        bat_low_level = DEFAULT_BAT_LOW;

    bat_level_delta = eeprom_read_byte(EEPROM_BAT_DELTA_ADDR);
    if (bat_level_delta == 0xFF)
        bat_level_delta = DEFAULT_BAT_DELTA;
...
static inline void battery_calibrate()
{
    ...
    {
        bat_low_level = bat;
        if (bat_low_level < bat_empty_level)
            bat_empty_level = bat_low_level;

        set_leds(LEDS_MODE_LOW);
        delay(100);
        bat = get_battery_direct();

        if (bat > bat_low_level)
            bat = bat_low_level;
...
static inline bool check_bat_level()
{
    ...
    case LEDS_MODE_LOW:
        ...
        break;
    case LEDS_MODE_TURBO:
        b += bat_level_delta * 2; /* continue */
    case LEDS_MODE_HIGH:
        b += bat_level_delta; /* continue */
    case LEDS_MODE_MED:
        b += bat_level_delta * 2;
        break;
    }
    ...
    { // expect only battery discharging
        return false;
    }
    else
    {
...
static inline void process_sos(uint8_t count)
{
    ...
    if (count & 0x07)
        return; // no changes, e.g. one unit ~ 0.4 sec
...
static inline void process_special_mode(uint8_t count)
{
    ...
    switch (leds_mode)
    {
    case LEDS_MODE_BEACON:
        process_beacon(count);
        break;
    case LEDS_MODE_STROBE:
        process_strobe(count);
        break;
    case LEDS_MODE_SOS:
        process_sos(count);
        break;
